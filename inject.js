  ...
  function make_s(renv, clazz_f, isout) {
    ...
    for (var i = 0; i < plist.length; i++) {
      try {
        ...
        if (is_literal(value)) {
          var _desc = Object.getOwnPropertyDescriptors(window[clazz].prototype)[
            plist[i]
          ];
          if (_desc) {
    ...
    if (e["config-hook-unescape"]) {
      unescape = saf(function unescape() {
        return e["config-hook-unescape"]
          ? v_logs("[unescape]:", arguments, v_unescape.apply(this, arguments))
...
chrome.storage.local.get(hookers, function (result) {
  ...
  if (result["config-myinject_toggle"]) {
    ...
    var myinject = check_format(myinject)
      ? myinject
      : 'console.log("format error.")';
    inject_script(
      (code_inject = `(${(inject_code + "").replace("$myinject", myinject)})()`)
    );
  }
});
