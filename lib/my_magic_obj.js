function my_magic_obj() {
  console.log("my_magic_obj", "启动");
  function make_v(envs, keys) {
          ...
          value: function () {
            ...
            for (var i = 0; i < _plugins.length; i++) {
              _ret.push(
                [
                  `  this[${i}]=v_new(Plugin);`,
                  `this[${i}].description=${JSON.stringify(
                    _plugins[i].description
                  )};`,
                  `this[${i}].filename=${JSON.stringify(
                    _plugins[i].filename
                  )};`,
            ...
            for (var i = 0; i < _mimeTypes.length; i++) {
              _ret.push(
                [
                  `  this[${i}]=v_new(Plugin);`,
                  `this[${i}].description=${JSON.stringify(
                    _mimeTypes[i].description
                  )};`,
                  `this[${i}].enabledPlugin=${JSON.stringify(
                    _mimeTypes[i].enabledPlugin
                  )};`,
                  `this[${i}].suffixes=${JSON.stringify(
                    _mimeTypes[i].suffixes
                  )};`,
    ...
    function make_return(clazz, name, value, type) {
      ...
      if (type != "get") {
        ...
        var val = val[1] || val[2];
        var prefix = `v_console_log("  [*] ${clazz} -> ${name}[${type}]", ${
          tog ? val : value
        })`;
      }
      return prefix + ";" + ret;
    ...
    function make_s(renv, clazz_f, isout) {
      ...
      for (var i = 0; i < lst.length; i++) {
        ...
        if (temp.func) {
          inner.push(
            `  ${name}: {value: v_saf(function ${name}(){${make_return(
              clazz,
              name,
              undefined,
              "func"
      ...
      var plist = Object.keys(
        (v_window_cache[clazz] || window[clazz]).prototype
      );
      ...
      for (var i = 0; i < plist.length; i++) {
        try {
          ...
          if (is_literal(value)) {
            var _desc = Object.getOwnPropertyDescriptors(
              window[clazz].prototype
            )[plist[i]];
            if (_desc) {
      ...
      inner.push(
        `  [Symbol.toStringTag]: {value:"${clazz}",writable:false,enumerable:false,configurable:true},`
      );
      if (inner.length) {
      ...
      var ls = [
        `${clazz} = v_saf(function ${clazz}(){${
          cannew
            ? ""
            : 'if (!v_new_toggle){ throw TypeError("Illegal constructor") }'
        };${init}})` + (father ? `; _inherits(${clazz}, ${father})` : ""),
      ];
      if (isout) {
        return [ls, inner];
      }
      ...
      for (var j = 0; j < temp.length; j++) {
        ...
        for (var k = 0; k < funcs.length; k++) {
          ...
          for (var l = 0; l < types.length; l++) {
            renv[temp[j]][funcs[k]][types[l]] =
              renv[temp[j]][funcs[k]][types[l]] || {};
            renv[temp[j]][funcs[k]][types[l]].value =
              e[temp[j]][funcs[k]][types[l]].value;
          }
        }
        ...
        if (idx == -1) {
          ...
          if (list[i] == "window") {
            ...
            if (/^[a-zA-Z_$][0-9a-zA-Z_$]*$/.test(list[i] + "")) {
              ...
              _global.push(
                `window[${JSON.stringify(list[i])}] = v_new(${name})`
              );
            }
          }
        } else {
    ...
    var tail = [
      ...
      `v_hook_href(window.location, 'location', ${JSON.stringify(
        location.href
      )})`,
      `Location.prototype.toString = v_saf(function toString(){ return ${JSON.stringify(
        location.href
      )} })`,
    ...
    var v_getele_inner = Object.keys(v_getele).map(function (e) {
      ...
      patcher(clzname);
      return `  if(name == ${JSON.stringify(
        e
      )} && func == ${JSON.stringify(v_getele[e][0])}){ return v_new(${clzname}) }`;
    });
    ...
    var v_geteles_inner = Object.keys(v_geteles).map(function (e) {
      ...
      return `  if(name == ${JSON.stringify(
        e
      )} && func == ${JSON.stringify(v_geteles[e][0])}){ return [${_clzs.join(",")}] }`;
    });
  ...
  function injectfunc(e, window) {
    ...
    if ((e["config-hook-regexp-url"] || "").trim()) {
      console.log(
        "[*] 配置了只收集对某个js路径才输出的配置（如出现控制台不输出hook，注意配置该项为空）:",
        e["config-hook-regexp-url"]
      );
    }
    // 备份 console.log
    function _mk_logs() {
          ...
          if (cache[a[0]] > log_limit) {
            if (!close_tog[a[0]]) {
              ...
              v_console_log(
                ...
                "超过接口的输出上限:",
                log_limit,
                ", 停止输出该接口的日志.（默认30，可在 “dom对象hook配置” 的红色窗口中配置）"
              );
            }
            return;
          }
    ...
    !(function () {
      ...
      delete n[t][e],
        ...
        (saf = function (n, m) {
          return (
            r(n, i, `function ${m ? m : n.name || ""}() { [native code] }`), n
          );
        });
    })();
    ...
    function openwin(txt) {
      var OpenWindow = window.open(
        "about:blank",
        "1",
        "height=600, width=800,toolbar=no,scrollbars=" + scroll + ",menubar=no"
      );
      ...
      function copyToClipboard(str) {
        try {
          ...
          if (selected) {
            ...
            alert(
              "保存至剪贴板失败。尝试直接将代码用 console.log 直接输出在控制台中。(因为可能会保存失败，可以多点几次 “生成临时环境”)"
            );
            console.log(str);
          }
        }
      ...
      !(function () {
        ...
        var _newFunction = function Function() {
          if (window.v_func) {
            ...
            if (e["config-hook-Function"]) {
              window.v_log(
                ..._mk_logs("Function code:", arguments[arguments.length - 1])
              );
            }
          }
      ...
      !(function () {
        ...
        eval = function eval() {
          if (window.v_eval) {
            ...
            if (e["config-hook-eval"]) {
              window.v_log(
                ..._mk_logs("eval code:", arguments[arguments.length - 1])
              );
            }
          }
          ...
          function replace_debugger(e) {
            if (/debugger/.test(e[e.length - 1])) {
              if (temp.indexOf(e[e.length - 1]) == -1) {
                ...
                window.v_log(
                  ..._mk_logs(
                    `[replace_debugger:${fname}]: debugger is exist, replace it with empty string.`
                  )
                );
              }
              ...
              if (temp.indexOf(e[e.length - 1]) == -1) {
                temp.push(e[e.length - 1]);
                window.v_log(
                  ..._mk_logs(`[replace_debugger:${fname}]: ${e[e.length - 1]}`)
                );
              }
            }
          }
    ...
    if (e["config-hook-cookie"]) {
      !(function () {
        // hook cookie get set
        var _old_cookie_get = Object.getOwnPropertyDescriptors(
          document.__proto__.__proto__
        ).cookie.get;
        var _old_cookie_set = Object.getOwnPropertyDescriptors(
          document.__proto__.__proto__
        ).cookie.set;
          ...
          if (window.v_cookie_get) {
            ...
            if (e["config-hook-cookie"] && e["config-hook-cookie-get"]) {
              ...
              if (expurl.v_test(expstr) && typeof expstr == "string") {
                window.v_log(
                  ..._mk_logs("(*) [cookie get]", r, get_log_at(expstr.trim()))
                );
              }
              if (
                e["config-hook-cookie-add-debugger"] &&
                r.indexOf(e["config-hook-cookie-match"]) != -1
              ) {
                debugger;
              }
            }
        ...
        var _new_cookie_set = function set(v) {
          if (window.v_cookie_set) {
            ...
            if (e["config-hook-cookie"] && e["config-hook-cookie-set"]) {
              ...
              if (expurl.v_test(expstr) && typeof expstr == "string") {
                window.v_log(
                  ..._mk_logs("(*) [cookie set]", v, get_log_at(expstr.trim()))
                );
              }
            }
            if (
              e["config-hook-cookie-add-debugger"] &&
              v.indexOf(e["config-hook-cookie-match"]) != -1
            ) {
              debugger;
            }
          }
      ...
      !(function () {
        ...
        setTimeout = function setTimeout() {
          if (window.v_settimeout) {
            ...
            if (e["config-hook-settimeout"]) {
              if (
                expurl.v_test((expstr = v_Error().stack.v_split("\n")[2])) &&
      ...
      if (e["config-hook-JSON.parse"]) {
        JSON.parse = saf(function parse() {
          return e["config-hook-JSON.parse"]
            ? v_logs("[JSON.parse]:", arguments, v_parse.apply(this, arguments))
      ...
      if (e["config-hook-escape"]) {
        escape = saf(function escape() {
          return e["config-hook-escape"]
            ? v_logs("[escape]:", arguments, v_escape.apply(this, arguments))
      ...
      if (e["config-hook-atob"]) {
        atob = saf(function atob() {
          return e["config-hook-atob"]
            ? v_logs("[atob]:", arguments, v_atob.apply(this, arguments))
      ...
      if (e["config-hook-btoa"]) {
        btoa = saf(function btoa() {
          return e["config-hook-btoa"]
            ? v_logs("[btoa]:", arguments, v_btoa.apply(this, arguments))
  ...
  var getsets = [
    ...
    ["webkitRTCPeerConnection", "sctp"],
    ["webkitRTCPeerConnection", "ondatachannel"],
    ["webkitRTCPeerConnection", "onaddstream"],
    ["webkitRTCPeerConnection", "onremovestream"],
    ["webkitMediaStream", "id"],
    ["webkitMediaStream", "active"],
    ["webkitMediaStream", "onaddtrack"],
    ["webkitMediaStream", "onremovetrack"],
    ["webkitMediaStream", "onactive"],
    ["webkitMediaStream", "oninactive"],
  ...
  chrome.storage.local.get(hookers, function (result) {
    ...
    var replacer_injectfunc = (injectfunc + "").replace(
      "$domobj_placeholder",
      make_domhooker_funcs()
    );
    var replacer_injectfunc = replacer_injectfunc.replace(
      "$make_v_func",
      make_v + ";"
    );
    var inject_code = `(${replacer_injectfunc})(${JSON.stringify(
      result
    )},window)`;
