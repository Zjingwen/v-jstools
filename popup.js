        ...
        chrome.tabs.query(
          ...
          function (tabs) {
            chrome.tabs.sendMessage(
              tabs[0].id,
              { action: { type: "logtoggle", info: "logtoggle" } },
              function (response) {}
            );
          }
        );
...
然后重新点击 “生成临时环境” 即可生成代码。`);
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      chrome.tabs.sendMessage(
        tabs[0].id,
        { action: { type: "addlistener", info: "addlistener" } },
        function (response) {}
      );
    });
  });
...
document
  .getElementById("create_high_env")
  .addEventListener("click", function (e) {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      function send(info) {
        chrome.tabs.sendMessage(
          tabs[0].id,
          { action: { type: "eval", info: info } },
          function (response) {}
        );
      }
  ...
  clone_page.addEventListener("click", function (e) {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      if (html) {
        ...
        alert(
          "获取html结构失败，请右键需要拷贝的页面的空白处，选择“打开 html 调试拷贝”。刷新页面后，确保页面资源加载充足后再重新点击“拷贝当前页面”"
        );
      }
    });
  });
