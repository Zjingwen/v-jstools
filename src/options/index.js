...
function _mk_html(input, clsname, index) {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    htmls.push(
      `<label style="margin-left: 40px; display:block" ><input class="${clsname}-e2" checked=true type="checkbox" data-key="config-hook-${k}-${v}" vilame="${index}">${k} ${v}<br /></label> `
    );
  }
...
document
  .getElementById("getsets_0")
  .addEventListener("change", changer.bind(null, "getsets_0", 0));
document
  .getElementById("funcs_0")
  .addEventListener("change", changer.bind(null, "funcs_0", 0));
document
  .getElementById("getsets_1")
  .addEventListener("change", changer.bind(null, "getsets_1", 1));
document
  .getElementById("funcs_1")
  .addEventListener("change", changer.bind(null, "funcs_1", 1));
...
var default_code_dyn_change_request = document.getElementById(
  "default_code_dyn_change_request"
);
...
var add_script_in_all_document = document.getElementById(
  "add_script_in_all_document"
);
...
function response_changer_init(idname, titlenames, init_data, callback) {
  ...
  title += "</tr>";
  title +=
    '<tr><td colspan="5"><button>添加行</button><button>添加示例1(重定向到本地文件)</button><button>添加示例2(直接修改返回值)</button></td></tr>';
  $(cid).html(title);
  ...
  function __cache_data() {
    ...
    while (data_list.length) {
      init_data.unshift(data_list.pop());
    }
    callback(init_data);
  }
...
chrome.storage.local.get(["response_changer"], function (res) {
  ...
  response_changer_init(
    "response_changer",
    ["(URL)字符串匹配", "重定向地址或修改的字符串", "类型|width:120px|true"],
    init_data,
    function (data) {
      chrome.storage.local.set({
