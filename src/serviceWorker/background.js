...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  ...
  if (menuItemId === "ast_hook") {
    // TODO 打开就闪退
    ast_dyn_hook = true;
    AttachDebugger();
  }
  if (menuItemId === "ast_hook_dyn") {
  ...
  if (menuItemId === "copy_page_resource") {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      if (html && html_copy) {
        var url = URL.createObjectURL(
          new Blob(html.split(""), { type: "text/javascript" })
        );
  ...
  if (menuItemId === "copy_page") {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      var url = tabs[0].url;
      var html = get_html(url);
      if (html && html_copy) {
        var url = URL.createObjectURL(
          new Blob(html.split(""), { type: "text/html" })
        );
        chrome.downloads.download({
          url: url,
...
function sendCommand(method, params, source, chainfun) {
  chrome.debugger.sendCommand(source, method, params, function (result) {
    if (chrome.runtime.lastError) {
      ...
      if (
        chrome.runtime.lastError.message.indexOf("Cannot access a chrome://") !=
        -1
      ) {
        close_debugger();
      }
            ...
            chrome.storage.local.get(["config-fetch_hook"], function (res) {
              ...
              if (html_copy) {
                if (
                  ...
                  if (params.resourceType == "Script") {
                    var save_info = decodeURIComponent(
                      escape(atob(result.body))
                    );
                  }
                  ...
                  if (params.resourceType == "Image") {
                    var save_info = result.body;
                  }
                  if (params.resourceType == "Font") {
                    var save_info = result.body;
                  }
                  if (params.resourceType == "Other") {
                    var save_info = result.body;
                  }
              ...
              if (ast_dyn_hook) {
                if (
                  ...
                  try {
                    var respboby = decodeURIComponent(
                      escape(atob(result.body))
                    );
                    var replacer = eval(
                      (res["config-fetch_hook"] || "") + ";fetch_hook"
                    );
                    if (params.resourceType == "Script") {
                      var replbody = replacer(respboby, params.request.url);
                    }
                    if (params.resourceType == "Document") {
                      var replbody = html_script_replacer(
                        respboby,
                        replacer,
                        params.request.url
                      );
...
function AttachDebugger() {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    ...
    chrome.debugger.attach(currtab, "1.2", function () {
      sendCommand("Network.enable", {}, currtab, function () {
        sendCommand(
          "Network.setCacheDisabled",
          { cacheDisabled: true },
          currtab
        );
      }); // 确保 Fetch.getResponseBody 一定能收到东西
      sendCommand(
        "Fetch.enable",
        {
          patterns: [
