...
sojsontn.addEventListener("click", function (e) {
  try {
    try {
      (txt2 || txt).value = muti_process_sojsondefusion(
        txt.value,
        get_ob_config()
      );
    } catch (e) {
...
obtn.addEventListener("click", function (e) {
  try {
    ...
    if (e.__proto__.name == "ReferenceError") {
      ...
      if (mth) {
        console.log(
          `出现 ReferenceError: '${mth[1]}' is not defined 的异常，尝试用 '${mth[1]}' 作为解密名字二次解密。`
        );
        var config = get_ob_config();
...
terserbtn.addEventListener("click", function (e) {
  ...
  terser.tail = "_";
  terser
    .minify(txt.value, { compress: false, output: { beautify: true } })
    .then(function (e) {
